...
```{r}
p1<-qplot(data = wine , x = quality,color = 'black', fill = '#F79420')
p2<-qplot(data = wine , x = rating,color = 'black', fill = '#ABB06')
...
We know pH measures acid concentration using a log scale, it is not surprise to find stronger correlations between pH the log of the acid concentrations. We can investigate how much of the variance in pH these three acidity variables can explain using a linear model.
...
```
Alcohol has a big influence on wine. So its an important factor to look for.
